		...
		for iLoopTeam in xrange(gc.getMAX_CIV_TEAMS()):
			if (gc.getTeam(iLoopTeam).isAlive() and not gc.getTeam(iLoopTeam).isMinorCiv()):
				...
				if iLoopTeam != iActiveTeam:
					nKings += gc.getTeam(iLoopTeam).getUnitClassCount(gc.getInfoTypeForString("UNITCLASS_KING"))
					if pTeam.isHasMet(iLoopTeam) or CyGame().isDebugMode():
...
## Ultrapack ##	
		...
		for iLoopVC in xrange(gc.getNumVictoryInfos()):
			...
			if CyGame().isVictoryValid(iLoopVC):
				...
				bEntriesFound = False
## Regicide ##
				if iLoopVC == gc.getInfoTypeForString("VICTORY_REGICIDE"):
					iRow = screen.appendTableRow(szTable)
					screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_ELIMINATE_ALL", ()), gc.getUnitCombatInfo(gc.getInfoTypeForString("UNITCOMBAT_KING")).getButton(), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					...
					screen.setTableText(szTable, 3, iRow, unicode(nKings), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					bEntriesFound = True
## Regicide ##				
				...
				for iLoopTeam in xrange(gc.getMAX_CIV_TEAMS()):
					if (gc.getTeam(iLoopTeam).isAlive() and not gc.getTeam(iLoopTeam).isMinorCiv()):
						if (iLoopTeam != iActiveTeam and (pTeam.isHasMet(iLoopTeam) or gc.getGame().isDebugMode())):
							teamBuilding = 0
				...
				if (victory.getCityCulture() != CultureLevelTypes.NO_CULTURELEVEL and victory.getNumCultureCities() > 0):
					...
					for iLoopPlayer in xrange(gc.getMAX_CIV_PLAYERS()):
						...
						if pLoopPlayer.isAlive() and not pLoopPlayer.isMinorCiv():
							if (pLoopPlayer.getTeam() != iActiveTeam and pTeam.isHasMet(pLoopPlayer.getTeam())) or CyGame().isDebugMode():
...
## Ultrapack ##
	def getListCultureCities(self, iPlayer):
		if iPlayer >= 0:
			...
			if pPlayer.isAlive():
				listCultureCities = pPlayer.getNumCities() * [(0, 0)]
				i = 0
				(loopCity, iter) = pPlayer.firstCity(False)
				while(loopCity):
					listCultureCities[i] = (loopCity.getCulture(iPlayer), loopCity)
					i += 1
