...
g_bStackSuperSpecialists = false
...
g_bDisplayUniqueSuperSpecialistsOnly = true
...
class CvMainInterface:
	...
	def interfaceScreen (self):
		...
		if(config != None):
			g_bHighlightForcedSpecialists = config.getboolean("Specialist Stacker", "Highlight Forced Specialists", True)

			SPECIALIST_STACK_WIDTH = config.getint("Specialist Stacker", "Specialist Stack Width", 15)

			g_bStackSuperSpecialists = config.getboolean("Specialist Stacker", "Stack Super Specialists", False)

			MAX_SUPER_SPECIALIST_BUTTONS = config.getint("Specialist Stacker", "Max Super Specialist Buttons", 6)

			SUPER_SPECIALIST_STACK_WIDTH = config.getint("Specialist Stacker", "Super Specialist Stack Width", 15)

			g_bDisplayUniqueSuperSpecialistsOnly = config.getboolean("Specialist Stacker", "Display Unique Super Specialists Only", true)

			g_bDynamicSuperSpecialistsSpacing = config.getboolean("Specialist Stacker", "Dynamic Super Specialists Spacing", true)

			g_bStackAngryCitizens = config.getboolean("Specialist Stacker", "Stack Angry Citizens", true)

			MAX_ANGRY_CITIZEN_BUTTONS = config.getint("Specialist Stacker", "Max Angry Citizen Buttons", 6)

			ANGRY_CITIZEN_STACK_WIDTH = config.getint("Specialist Stacker", "Angry Citizen Stack Width", 15)

			g_bDynamicAngryCitizensSpacing = config.getboolean("Specialist Stacker", "Dynamic Angry Citizen Spacing", true)
...
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=1/9===0===
############################################################
		screen.addPanel( "CityScreenAdjustPanelb", u"", u"", True, False, 10, 154, 238, 88, PanelStyles.PANEL_STYLE_STANDARD )
		screen.setStyle( "CityScreenAdjustPanelb", "Panel_City_Info_Style" )
		screen.hide( "CityScreenAdjustPanelb" )
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=1/9===X===
############################################################
		...
		for i in range( gc.getNumSpecialistInfos() ):
			...
			if (gc.getSpecialistInfo(i).isVisible()):
				...
				screen.setButtonGFC( szName, u"", "", xResolution - (38+iXShiftVal), (yResolution - 273 - (30 * iYShiftVal)), 19, 19, WidgetTypes.WIDGET_CHANGE_SPECIALIST, i, 1, ButtonStyles.BUTTON_STYLE_CITY_PLUS )
				screen.hide( szName )
		...
		for i in range( gc.getNumSpecialistInfos() ):
			...
			if (gc.getSpecialistInfo(i).isVisible()):
				...
				screen.setButtonGFC( szName, u"", "", xResolution - (38+iXShiftVal), (yResolution - 258 - (30 * iYShiftVal)), 19, 19, WidgetTypes.WIDGET_CHANGE_SPECIALIST, i, -1, ButtonStyles.BUTTON_STYLE_CITY_MINUS )
				screen.hide( szName )
		...
		for i in range( gc.getNumSpecialistInfos() ):
			...
			if (gc.getSpecialistInfo(i).isVisible()):
				...
				screen.setImageButton( szName, gc.getSpecialistInfo(i).getTexture(), xResolution - (74+iXShiftVal), (yResolution - 271 - (30 * iYShiftVal)), 30, 30, WidgetTypes.WIDGET_DISABLED_CITIZEN, i, -1 )
				screen.enable( szName, False )
				screen.hide( szName )

				for j in range(MAX_CITIZEN_BUTTONS):
					szName = "CitizenButton" + str((i * 100) + j)
					screen.addCheckBoxGFC( szName, gc.getSpecialistInfo(i).getTexture(), "", xResolution - (74+iXShiftVal) - (SPECIALIST_STACK_WIDTH * j), (yResolution - 271 - (30 * iYShiftVal)), 30, 30, WidgetTypes.WIDGET_CITIZEN, i, j, ButtonStyles.BUTTON_STYLE_LABEL )
					screen.hide( szName )
				iCount = iCount + 1
			...
			screen.setLabel( "SpecialistLabel", "Background", "Specialists", CvUtil.FONT_CENTER_JUSTIFY, xResolution - 128, yResolution-447, -0.1, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
			screen.hide( "SpecialistLabel" )

		##############################
		# Rise of Mankind <STACKER END>
		##############################
...
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=2/9===0===
############################################################
...
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=2/9===X===
############################################################
	...
	def updateScreen(self):
		...
		g_iPlagueEffectTimer += 1
		if g_iPlagueEffectTimer == 12:
			g_iPlagueEffectTimer = 0
			for iCoords in pGodsOfOld.lPlagueCities:
				city = gc.getMap( ).plot( iCoords[0], iCoords[1] ).getPlotCity( )				
				cityPlotPoint = city.plot( ).getPoint( )
				plagueEffect = CvUtil.findInfoTypeNum( gc.getEffectInfo, gc.getNumEffectInfos( ), "EFFECT_GODS_PLAGUE" )
			...
			if (CyInterface().getInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_SELECTION):
				...
				if ( pHeadSelectedUnit.getOwner() == gc.getGame().getActivePlayer() and g_pSelectedUnit != pHeadSelectedUnit ):
					...
					if pUnitOwner.isTurnActive( ):
						...
						# Doctor
						if iUnitType == CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), "UNIT_GREAT_DOCTOR"):

								screen.appendMultiListButton( "BottomButtonContainer", ArtFileMgr.getInterfaceArtInfo("INTERFACE_GODS_PLAGUE").getPath(), 0, WidgetTypes.WIDGET_GENERAL, 668, 668, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1

						# Diplomat
						if iUnitType == CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), "UNIT_STATESMAN"):

							screen.appendMultiListButton( "BottomButtonContainer", ArtFileMgr.getInterfaceArtInfo("INTERFACE_GODS_METEOR").getPath(), 0, WidgetTypes.WIDGET_GENERAL, 669, 669, False )
							screen.show( "BottomButtonContainer" )
							iCount = iCount + 1

							b_bath = gc.getInfoTypeForString("BUILDING_GREAT_BATH")
							obsoleteTech = gc.getBuildingInfo(b_bath).getObsoleteTech()
							button = true

							if ( gc.getTeam(pUnitOwner.getTeam()).isHasTech(obsoleteTech) == false or obsoleteTech == -1 ):
								for iCity in range(pUnitOwner.getNumCities()):
									ppCity = pUnitOwner.getCity(iCity)
									if ppCity.getNumActiveBuilding(b_bath) == true:

										button = false

										#screen.appendMultiListButton( "BottomButtonContainer", ArtFileMgr.getInterfaceArtInfo("INTERFACE_GODS_EARTHQUAKE").getPath(), 0, WidgetTypes.WIDGET_GENERAL, 666, 666, False )
										#screen.show( "BottomButtonContainer" )
										#Count = iCount + 1
						...
						# Andromeda
						if iUnitType == CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), "UNIT_ANDROMEDA"):
								
								screen.appendMultiListButton( "BottomButtonContainer", ArtFileMgr.getInterfaceArtInfo("INTERFACE_GODS_BLIGHT").getPath(), 0, WidgetTypes.WIDGET_GENERAL, 670, 670, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1

						# Inquisitor
						if iUnitType == CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), "UNIT_INQUISITOR") and gc.getMap().plot( g_pSelectedUnit.getX(), g_pSelectedUnit.getY() ).isCity():
								...
								if iStateReligion != -1:
									if pCity.isHasReligion( iStateReligion ):
										for iReligionLoop in range(gc.getNumReligionInfos( )):
											...
											if (pCity.isHolyCityByType(iReligionLoop) == False):
						...
						if iUnitType == CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), "UNIT_LAWYER") and gc.getMap().plot( g_pSelectedUnit.getX(), g_pSelectedUnit.getY() ).isCity():
							...
							if ( pCity.getOwner( ) == pUnit.getOwner( ) ) or ( gc.getTeam( pCityPlayer.getTeam( ) ).isVassal( gc.getPlayer( pUnit. getOwner( ) ).getTeam( ) ) ): 
								...
								for iCorpLoop in range(gc.getNumCorporationInfos( )):
									...
									if (pCity.isHeadquartersByType(iCorpLoop) == False):

										if pCity.isHasCorporation( iCorpLoop ):

											screen.appendMultiListButton( "BottomButtonContainer", ArtFileMgr.getInterfaceArtInfo("INTERFACE_GODS_TSUNAMI").getPath(), 0, WidgetTypes.WIDGET_GENERAL, 667, 667, False )
											screen.show( "BottomButtonContainer" )
											iCount = iCount + 1
											return
	...
	def updateCitizenButtons( self ):
		...
		if ( CyInterface().isCityScreenUp() ):
			...
			if (pHeadSelectedCity and CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_SHOW):
				...
				if(g_bStackAngryCitizens == false):
				...
				for i in range(currentAngryCitizenCount):
					bHandled = True
					szName = "AngryCitizen" + str(i)
					screen.setImageButton( szName, ArtFileMgr.getInterfaceArtInfo("INTERFACE_ANGRYCITIZEN_TEXTURE").getPath(), xResolution - 74 - (stackWidth * i), yResolution - 238, 30, 30, WidgetTypes.WIDGET_ANGRY_CITIZEN, -1, -1 )
					screen.show( szName )
				...
				for i in range(gc.getNumSpecialistInfos()):
					for j in range( pHeadSelectedCity.getFreeSpecialistCount(i) ):
						...
						screen.setImageButton( szName, gc.getSpecialistInfo(i).getTexture(), (xResolution - 74  - (stackWidth * iCount)), yResolution - 208, 30, 30, WidgetTypes.WIDGET_FREE_CITIZEN, i, 1 )
						screen.show( szName )
						bHandled = true
				...
				for i in range( gc.getNumSpecialistInfos() ):
					...
					if( iSpecialistCount > 5 ):
						...
						# CvUtil.pyPrint('iYShiftVal =: %f' %iYShiftVal)
					else:
					...
					while(j >= 0):
						...
						if (gc.getSpecialistInfo(i).isVisible()):
							...
							screen.addCheckBoxGFC( szName, gc.getSpecialistInfo(i).getTexture(), "", xResolution - 74 - (SPECIALIST_STACK_WIDTH * j), (yResolution - 271 - (30 * i)), 30, 30, WidgetTypes.WIDGET_CITIZEN, i, j, ButtonStyles.BUTTON_STYLE_LABEL )

						screen.show( szName )
						szName = "CitizenButtonHighlight" + str((i * 100) + j)
						if (gc.getSpecialistInfo(i).isVisible()):
							screen.addDDSGFC( szName, ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), xResolution - (74+iXShiftVal) - (SPECIALIST_STACK_WIDTH * j), (yResolution - 271 - (30 * iYShiftVal)), 30, 30, WidgetTypes.WIDGET_CITIZEN, i, j )
						else:
							screen.addDDSGFC( szName, ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), xResolution - 74 - (SPECIALIST_STACK_WIDTH * j), (yResolution - 271 - (30 * i)), 30, 30, WidgetTypes.WIDGET_CITIZEN, i, j )
...
############################################################
##########=MSM===Ñombat XP Counter (NeverMind)===1/2===0===
############################################################
		screen.hide( "CombatXPButton" )
		screen.hide( "CombatXPText" )
############################################################
##########=MSM===Ñombat XP Counter (NeverMind)===1/2===X===
############################################################
		screen.hide( "ResearchBar" )
...
############################################################
##########=MSM===Ñombat XP Counter (NeverMind)===2/2===0===
############################################################
				if g_bCombatXPCounter and gc.getPlayer(ePlayer).getCombatExperience() > 0:
					eCombatXPText = " " + unicode(gc.getPlayer(ePlayer).getCombatExperience()) + "/" + unicode(gc.getPlayer(ePlayer).greatPeopleThreshold(true))
					screen.setImageButton( "CombatXPButton", ArtFileMgr.getUnitArtInfo("ART_DEF_UNIT_WARLORD_ANCIENT").getButton(), 199, 46, 24, 24, WidgetTypes.WIDGET_GENERAL, -1, -1 )
...
############################################################
##########=MSM===Ñombat XP Counter (NeverMind)===2/2===X===
############################################################
	...
	def updateCityScreen( self ):
		...
		screen.hide( "CultureBar" )
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=3/9===0===
############################################################
...
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=3/9===X===
############################################################
...
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=4/9===0===
############################################################			
				screen.show( "CityScreenAdjustPanelb" )
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=4/9===X===
############################################################
...
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=5/9===0===
############################################################
				...
				screen.addTableControlGFC( "BuildingListTable", 3, 10, 407, 238, yResolution - 631, False, False, 32, 32, TableStyles.TABLE_STYLE_STANDARD )
				screen.setStyle( "BuildingListTable", "Table_City_Style" )
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=5/9===X===
############################################################
...
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=6/9===0===
############################################################	
				iNumTradeRoutes = 0
				iNetTradeAmount = 0
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=6/9===X===
############################################################
...
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=7/9===0===
############################################################
						iNumTradeRoutes = iNumTradeRoutes + 1
						iNetTradeAmount += iTradeProfit
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=7/9===X===
############################################################
...
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=8/9===0===
############################################################
				szBuffer = "Plot Raw Commerce:"
				screen.setLabel( "MaintenanceTextb", "Background", szBuffer, CvUtil.FONT_LEFT_JUSTIFY, 15, 160, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				screen.show( "MaintenanceTextb" )

				szBuffer = localText.getText("TXT_KEY_HEADING_TRADEROUTE_LIST", ()) + ":"
				screen.setLabel( "MaintenanceTextc", "Background", szBuffer, CvUtil.FONT_LEFT_JUSTIFY, 15, 180, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				screen.show( "MaintenanceTextc" )

				szBuffer = localText.getText("TXT_KEY_CONCEPT_BUILDINGS", ()) + localText.getText("TXT_KEY_AND", ()) + localText.getText("TXT_KEY_CONCEPT_SPECIALISTS", ()) + ":"
				screen.setLabel( "MaintenanceTextd", "Background", szBuffer, CvUtil.FONT_LEFT_JUSTIFY, 15, 200, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				screen.show( "MaintenanceTextd" )

				szBuffer = "<color=205,180,55,255>Net Raw Commerce:</color>"
				screen.setLabel( "MaintenanceTexte", "Background", szBuffer, CvUtil.FONT_LEFT_JUSTIFY, 15, 220, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				...
				# Show Plot Commerces
				szBuffer = u"%s%d %c" %( "+", pHeadSelectedCity.getYieldRate(2) - myLittleYield - pHeadSelectedCity.getExtraSpecialistYield(2)-iNetTradeAmount, gc.getYieldInfo(2).getChar())
				screen.setLabel( "MaintenanceTextf", "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, 225, 160, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				screen.show( "MaintenanceTextg" )
				
				# Show  trade route commerces
				szBuffer = u"%s%d %c" %( "+", iNetTradeAmount, gc.getYieldInfo(2).getChar() )
				screen.setLabel( "MaintenanceTextg", "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, 225, 180, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				screen.show( "MaintenanceTextf" )

				# Show Modifiers
				szBuffer = u"%s%d %c" %( "+", myLittleYield + pHeadSelectedCity.getExtraSpecialistYield(2), gc.getYieldInfo(2).getChar())
				screen.setLabel( "MaintenanceTexth", "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, 225, 200, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				screen.show( "MaintenanceTextf" )
	
				# Show Total Net Commerce
				szBuffer = u"<color=205,180,55,255>%s%d</color> %c" %( "=", pHeadSelectedCity.getYieldRate(2), gc.getYieldInfo(2).getChar() )
				screen.setLabel( "MaintenanceTexti", "Background", szBuffer, CvUtil.FONT_RIGHT_JUSTIFY, 225, 220, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				screen.show( "MaintenanceTextg" )
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=8/9===X===
############################################################
...
#					xCoord = xResolution - 242 + ((i % 14) * (16 + -0))
#					# xCoord = xResolution - 242 + (i * 30)
#					yCoord = 42 + 16 * (i // 14)					
#					# yCoord = 42
#					
...
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=9/9===0===
############################################################
			screen.hide( "CityScreenAdjustPanelb" )
############################################################
##########=MSM===Raw City Commerce Display (Sevo)=9/9===X===
############################################################
...
#attitude icons - end				
	...
	def handleInput (self, inputClass):
		...
		if (inputClass.getNotifyCode() == 11 and inputClass.getData1() == 667 and inputClass.getData2() == 667):
			...
			iMessageID = pGodsOfOld.m_iNetMessage_ProphetEnki
			iPlotX = pPlot.getX()
			iPlotY = pPlot.getY()
		...
		if (inputClass.getNotifyCode() == 11 and inputClass.getData1() == 665 and inputClass.getData2() == 665):
			...
			iMessageID = pGodsOfOld.m_iNetMessage_Inquisitor
			iPlotX = pPlot.getX()
			iPlotY = pPlot.getY()
			iOwner = g_pSelectedUnit.getOwner()
			iUnitID = g_pSelectedUnit.getID()
