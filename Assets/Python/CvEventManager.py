...
## Terraform Begin (avain)
		...
		if(iImprovement==gc.getInfoTypeForString('IMPROVEMENT_HILL')):
			pPlot = CyMap().plot(iX, iY)
			if (pPlot.getPlotType()==PlotTypes.PLOT_HILLS):
				CyInterface().addMessage(CyGame().getActivePlayer(),True,25,'Levelled terrain!','AS2D_DISCOVERBONUS',1,'Art/Interface/Buttons/btn_build_hill.dds',ColorTypes(8),iX,iY,True,True)
				pPlot.setPlotType(PlotTypes.PLOT_LAND, True, True)
			else:
				CyInterface().addMessage(CyGame().getActivePlayer(),True,25,'Hill created!','AS2D_DISCOVERBONUS',1,'Art/Interface/Buttons/btn_build_hill.dds',ColorTypes(8),iX,iY,True,True)
				pPlot.setPlotType(PlotTypes.PLOT_HILLS, True, True)
			pPlot.setImprovementType(-1)
			
		if(iImprovement==gc.getInfoTypeForString('IMPROVEMENT_TERRAFORMED_TERRAIN')):
			pPlot = CyMap().plot(iX, iY)
			CyInterface().addMessage(CyGame().getActivePlayer(),True,25,'Terraforming completed!','AS2D_DISCOVERBONUS',1,'Art/Interface/Buttons/btn_terraform.dds',ColorTypes(8),iX,iY,True,True)
